...
**브랜치는 `feat/shadow-coach` 그대로.** 4.4의 **구조 커밋이 들어갔다** — 무대 → 슬롯 → 저장이 한 흐름으로 돈다.
관문 전부 초록 — `tsc` 0 · `eslint` 0 · `jest` 146/146 (11묶음) · `ait build` 0/0 양쪽 RN.

들어간 것:

- `comboDraft` 리듀서 ([hooks/comboDraft.ts](../src/screens/ShadowCoach/hooks/comboDraft.ts)) — 무대·슬롯·한 줄 입력이 한 상태.
  `index.tsx`의 `draft`/`editingId`/`comboHint` 세 `useState`가 이걸로 합쳐졌다. 순수 테스트 14개
- `TapStage`(무대, 고정 180px) · `SlotRow`(자리 줄, 콤보 탭과 고르기 시트가 같이 쓴다). `ComboChips`는 지웠다
- `CombosView`는 무대·자리 줄을 **스크롤 밖**에 둔다. 두드리는 동안 목록은 `pointerEvents="none"` + 흐리게
- 저장 FAB은 슬롯 단계에서 뜬다. 막는 이유는 넘침 → 빈 자리(미인식이면 그 안내) → 중복 순
- 듣기(초안): 채운 자리는 말, 빈 자리는 `blip`. 탭마다 `voice.tick()` 햅틱
- 시트로 채우거나 자리를 비우면 **입력창을 비운다** — 빈 자리를 글로 못 적어서. 기획서 4.4에 그렇게 고쳤다
- 자료 리듀서 `addCombo`/`replaceCombo`가 `rhythm`을 받는다. 자리 하나짜리는 리듬을 안 싣는다

**다음: 모양 커밋.** 결정된 것 — 그림 없이, 두드릴 때 **탭 수 숫자 + 무대가 살짝 움직임**(약 100ms, `Animated` + `useNativeDriver`).
숫자는 이미 있다(`DISPLAY.done` 48px을 빌렸다). 움직임만 남았다. 그다음 **기기에서 본다**:

- 첫 탭 지연 · 햅틱이 손가락과 맞는가 · 무대 밖으로 흐른 손이 목록을 안 건드리는가
- 슬롯 단계에서 키보드가 올라올 때 자리 줄이 안 밀리는가(스크롤 밖이라 안 밀려야 한다)
- 고르기 시트에서 자리 상자 키가 그대로인가, 마지막을 채우면 닫히는가
- 듣기 — 빈 자리의 클릭음과 말이 두드린 간격대로 나오는가
- **말 속도 기본값(1.15)을 올릴지**는 B(초기 데이터)에서 정한다. 리듬 콤보가 제대로 들리려면 높아야 한다

D(색)는 여전히 열려 있다. 무대·자리 줄은 지금 팔레트(`C.card`/`C.line`/`ACCENT`)로 그렸다 — D가 정해지면 같이 바뀐다.
...
**구조는 들어갔다** (맨 위 인계 참고). 다음은 모양 커밋과 기기 확인이다.
...
- [ ] 파서를 걷어내는 기획이면 **테스트도 같이 없어진다.** 지금 146개 중 32개가 통째로 그것이다.
