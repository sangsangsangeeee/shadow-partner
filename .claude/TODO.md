...
**브랜치는 `feat/shadow-coach` 그대로. 마지막 커밋 `978d184`, 그 위에 미커밋 변경이 있다.**
커밋을 셋으로 나눌 것: 문서(테스트 수) / 모델·식 승격(`rhythm` · `comboSteps` · 호출부 셋 · `steps.test`) / 기획서.

관문 전부 초록 — `tsc` 0 · `eslint` 0 · `jest` 131/131 (10묶음) · `ait build` 0/0 양쪽 RN.

**스파이크는 끝났고 씨앗은 걷었다.** 기기에 `spike-*` 카드가 남아 있으면 손으로 지운다(콤보 탭에서 삭제).

결과 — **하한은 코드가 아니라 말 속도가 정한다.** 잽 4연타를 들었더니 말 속도 1.15에서는 0.2·0.3·0.4가
같게 들리고 0.5부터 갈렸는데, **2.0에서는 넷이 다 갈렸다.** TTS가 앞 말을 끊지 않고 줄을 세워서 그렇다.
그래서 배율도 하한도 안 건다 — 두드린 대로 저장·재생하고, 저장 전에 실제 목소리로 들려준다.
기획서 7장·12장에 그대로 적었다.

**기획서를 고쳤다** — 2장(원칙 3의 자리) · 3장 · 4.1 · **4.4·4.5 통째** · 5장(`rhythm`) · 6장 · 7장 · 9장 · 10장 · 11장 · 12장.
이제 기획서가 코드보다 앞서 있다. 코드는 재생 쪽(`comboSteps`)만 따라와 있다.

**다음: 4.4를 만든다.** 이 순서로, 구조 커밋과 모양 커밋을 나눠서:

1. **두드리는 무대** — 콤보 탭 상단 고정 높이. `nativeEvent.timestamp`로 재고, 탭마다 햅틱, 두드리는 동안 목록을 죽인다.
   시작은 첫 터치, 끝은 완료 버튼. 훅 하나(`useTapRhythm`쯤)와 부품 하나. **손맛은 기기에서만 안다**
2. **슬롯 줄** — 탭 수만큼 빈 자리. 시트(4.5)로 하나씩, 또는 한 줄 입력으로 순서대로. 슬롯 ↔ 입력창 역동기화는
   지금 칩 ↔ 입력창 그대로다. 넘침 안내 · 비우기 · 다시 두드리기 · 듣기(채운 건 말, 빈 건 클릭)
3. **콤보 탭 재구성** — [CombosView](../src/screens/ShadowCoach/views/CombosView.tsx) 291줄이 거의 다시 쓰인다.
   `screen.test`의 콤보 탭 테스트가 여럿 깨질 것이고 같이 다시 쓴다. 검증 시나리오는 11장의 `tap` `slot` `rhythm`
4. 말 속도 기본값 1.15를 올릴지는 **B(초기 데이터)에서** 정한다 — 리듬 콤보가 제대로 들리려면 높아야 한다

D(색)는 여전히 열려 있다. 4.4를 새로 그리는 김에 D의 답이 있으면 한 번에 그린다 — 없으면 지금 팔레트로 만들고 나중에 바꾼다.
...
**재생 쪽과 기획서는 끝났다** (맨 위 인계 참고). 다음은 무대·슬롯·탭 재구성이다.
...
- [ ] 파서를 걷어내는 기획이면 **테스트도 같이 없어진다.** 지금 131개 중 32개가 통째로 그것이다.
