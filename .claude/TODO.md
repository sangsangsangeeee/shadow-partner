...
1. **main 병합.** `preview.tsx`는 지웠다(3-3 완료).
   **다음 판(아래 A~D)을 시작하기 전에 매듭짓기를 권한다** — 검증이 끝난 상태를 한 번 못 박아 두면
   새 작업이 깨졌을 때 어디로 돌아갈지가 분명해진다. A~D는 새 브랜치에서.
2. **다음 판 — 새로 들어온 일 넷 (A~D).** 아래 절에 따로 적었다
