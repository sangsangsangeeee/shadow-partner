...
## 내일 여기서 시작한다 (2026-09-06 마감 기준)

**브랜치는 `feat/shadow-coach` 그대로 간다. 병합은 미뤘다** — 새 판(A~D)을 여기서 이어서 한다.
작업 트리는 깨끗하고, 로컬이 `origin/feat/shadow-coach`보다 커밋 하나 앞서 있다.
마지막 커밋은 `a992267`.

관문은 전부 초록이다 — `tsc` 0 · `eslint` 0 · `jest` 126/126 · `ait build` 0 errors / 0 warnings 양쪽 RN.
