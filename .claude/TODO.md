...
1. **백그라운드 일시정지를 기기에서 본다.** 코드와 테스트는 있고 실물만 안 봤다 (2번 안쪽)
2. **저장소 껐다 켜기.** 전체 점검에 남은 마지막 한 칸 (2번 안쪽)
3. **기획서에 백그라운드 동작을 적는다.** 기획서에 없는 동작이 코드에 생겼다.
   기획서가 권위인데 코드가 앞서 있는 상태다 — 병합 전에 뒤집어야 한다
4. **main 병합 + [preview.tsx](../src/preview.tsx) 2289줄 삭제** (3-3)
5. 그 뒤에 남는 건 취향의 문제뿐이다 — `Segmented` 글자 1px, 3번의 구조 정리 둘

**1·2·3이 끝나면 병합이다.** 1번과 2번은 기기가 필요하고, 3번은 지금 할 수 있다.
...
- [x] 기획서 11장의 14개 시나리오를 기기에서 한 번씩 (2026-09-06) —
...
- [ ] 백그라운드 일시정지 (아래에서 나온 것, 코드는 들어갔고 실물만 안 봤다)

**저장소는 세 갈래 중 둘이 개발 서버로 된다.**
...
- [ ] **iOS에서 나갔다 오면 멈춰 있는지.** 그리고 돌아오자마자 호출어가 쏟아지지 않는지
- [ ] **알림 배너·제어 센터로는 안 멈추는지.** `inactive`를 걸렀지만 실물 감각은 다르다
- [ ] **안드로이드에서도 같은지.** 거기서는 원래 계속 돌았을 수도 있다 — 그렇다면 이 결정을 다시 잰다
- [ ] 멈춘 뒤 재개했을 때 콤보를 처음부터 부르는지 (기획서 101줄)
