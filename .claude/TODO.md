...
## 내일 여기서 시작한다 (2026-09-11 마감 기준)

**브랜치는 `feat/shadow-coach` 그대로 간다. 병합은 미뤘다** — 새 판(A·C·D)을 여기서 이어서 한다.
작업 트리는 깨끗하고 `origin/feat/shadow-coach`와 같은 자리다. main보다 43커밋 앞서 있다.

관문은 전부 초록이다 — `tsc` 0 · `jest` 126/126 (2026-09-11 다시 확인).
`eslint` 0 · `ait build` 0 errors / 0 warnings 양쪽 RN은 2026-09-06 기록이고, 그 뒤로 코드를 안 건드렸다.

**B(초기 데이터)는 닫혔다 — 안 바꾸기로 했다.** 남은 셋은 전부 기획·디자인 판단을 기다린다.

**막고 있는 것: D의 "너무 동적"이 무엇을 가리키는지 모른다.** 답에 따라 손댈 곳이 달라진다:

| 답 | 손댈 곳 |
|---|---|
| 움직임·전환이 많아 산만하다 | 애니메이션 — `Motion.tsx`, 스와이프, 시트 스프링 |
| 눌리는 것과 배경이 구분이 안 된다 | **팔레트가 아니라 층 구조.** `C.sheet` #101013 · `C.card` #17171c · `C.line` #202027이 서로 너무 가깝다 |
| 액센트 색 자체가 마음에 안 든다 | `ACCENT`와 기획서 2장 |

**색을 바꾸는 일부터 하면 A와 C를 새 색으로 한 번에 만든다.** 그래서 D의 답이 먼저다.

시작 순서 제안:

1. D의 질문에 답을 받는다 → 기획서 2장을 고친다 → 팔레트나 층 구조를 손본다
2. C의 새 기획을 기획서 4.4·4.5에 쓴다. **대규모라 여기가 제일 오래 걸린다**
3. A는 C·D가 정해지면 그 위에서 한다
...
1. **다음 판 — 남은 셋 (A·C·D).** 아래 절에 따로 적었다. B는 안 바꾸기로 닫혔다
...
**B는 닫혔다(안 바꾼다). 남은 셋은 기획이 먼저다.** 특히 C와 D는 지금 기획서와 정면으로 어긋나므로,
**코드보다 [기획서](../docs/shadow-partner-spce.md)를 먼저 고쳐야 한다** — 어긋나면 기획서가 이긴다.

순서를 정하는 축이 하나 더 늘었다. **D(색)를 먼저 하면 A·C를 그 색으로 한 번에 만든다.**
D를 나중에 하면 A와 C를 두 번 손대게 된다. **B는 이 축과 무관해서 먼저 닫을 수 있었다** —
초기 데이터는 색도 콤보 기획도 안 탄다.
...
### B. 초기 데이터 변경 — **안 바꾼다. 2026-09-11에 닫혔다**

**지금 값을 그대로 둔다.** 세 갈래를 놓고 보고 셋 다 손대지 않기로 했다 —
[useMaterial.ts:33](../src/screens/ShadowCoach/hooks/useMaterial.ts#L33)의 `INITIAL_COMBOS` 넷
(`잽-크로스-로우킥` / `잽-잽-크로스-레프트훅` / `잽-미들킥` / `잽-크로스-슬립-크로스`),
[moves.ts](../src/commons/constants/moves.ts)의 `BASE_MOVES` 20개(펀치 7 · 킥 5 · 방어 4 · 풋워크 4),
`DEFAULTS` 열 값(3라운드 · 180초 · 휴식 60초 · 템포 1.0 · 간격 1.6 · 불규칙 켬 · 랜덤 · 8회 · 말속도 1.15).

**기획서가 이 값들을 못 박은 적이 없다.** 7장이 리듬 척도를, 2장이 색을 직접 정한 것과 다르다.
그래서 이건 기획서를 고쳐야 하는 일이 아니었고, 코드에서 바로 바꿀 수 있는 것이었다.
바꿀 이유가 없다는 것이 결론이다.

**되살리자는 제안이 나오면 아래 둘이 여전히 걸린다. 다시 재지 마라:**

- **이미 쓰고 있는 사람의 자료는 안 바뀐다.** 초기값은 저장소가 빈 첫 실행에만 쓰인다.
  바꾼 값을 기존 사용자에게도 주려면 그건 초기값이 아니라 **마이그레이션**이고 별개의 일이다 —
  버전 키와 병합 규칙이 필요하고, 사용자가 고친 것을 덮어쓰지 않게 설계해야 한다.
- `BASE_MOVES`의 id를 바꾸면 저장된 `labels`·`beats`·`combos`가 그 id를 가리키고 있다.
  **id는 자료의 열쇠다. 이름만 바꾸는 것과 id를 바꾸는 것은 전혀 다른 일이다.**
  이름·별칭·기본 리듬은 id를 그대로 두면 안전하게 바꿀 수 있다.

### C. 콤보 추가 — 기획 변경에 따른 대규모 변경

**아직 무엇으로 바뀌는지 안 적혀 있다. 그것부터다.**
...
- [ ] 지금 걸려 있는 것 — [CombosView](../src/screens/ShadowCoach/views/CombosView.tsx) 291줄 ·
      ...
      [parser.ts](../src/commons/utils/parser.ts) 167줄, 그리고 파서 테스트 13가지 입력 형식
- [ ] 파서를 걷어내는 기획이면 **테스트도 같이 없어진다.** 지금 97개 중 한 묶음이 통째로 그것이다.
...
| 초기 데이터 변경 | 지금 값이 괜찮다. 기획서가 못 박은 적도 없다 — B |
