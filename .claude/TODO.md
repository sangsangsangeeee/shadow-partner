...
## 여기서 시작한다 (2026-09-15 마감 기준)

**브랜치 `feat/shadow-coach`, `bc3a642`. 원격과 같은 자리. 작업 트리 깨끗.**
관문 전부 초록 — `tsc` 0 · `eslint` 0 · `jest` 91/91 (10묶음) · `ait build` 0/0 양쪽 RN.

**코드가 v2로 섰다.** 1번(지우기)과 2번(무대·이름·카드)이 닫혔다 —
파서·동작·호출어 탭·두드리기·슬롯·TTS가 전부 나갔고, 탭은 훈련·콤보 둘, 콤보는 이름과 녹음만 갖는다.

**막고 있는 것: 기기 확인이다.** 아래 4번이 그것이고, **여기가 다음 차례다.**
3번(훈련 화면)은 모양 맞추기만 남았는데 그 여백을 기기가 정한다 — 그래서 4번이 먼저다.

**기기가 정해 줘야 코드가 바뀌는 셋:** 문턱 5%(4-2) · 템포 범위(4-7) · 저장소 한도(4-8).
나머지 여섯은 되는지만 본다.

**확인 안 된 것을 확인됐다고 적지 마라.** 아홉 가지 전부 아직 안 봤다.
...
## 3. 훈련 화면 커밋 — **거의 다 됐다. 남은 건 4번 뒤에**

1번에서 뼈대가 다 들어갔다. 아래 넷은 확인만 하면 된다:

- [x] `TrainView` 이름 + 막대 — 1번에서 들어갔다(`clipPlan(...).total`로 막대가 찬다)
- [x] `useCallouts` 본체 없는 콤보 건너뛰기 — 1번에서 들어갔다
- [x] `DoneOverlay` 3행 — 동작 수 행을 걷었다(기획서 4.3)
- [x] 설정 시트 `format`이 두 자리 — `v.toFixed(2)`로 맞다

**남은 것은 모양 하나뿐이고, 그 여백은 기기가 정한다.**
칩과 비트 트랙이 있던 자리가 비었는데 얼마나 비울지는 눈으로 봐야 안다.
**요청 안 한 간격은 건드리지 않는다**(design-system.md) — 4번에서 보고 나서 손댄다.

---

## 4. 기기 확인 — **여기가 다음이다. 아홉 가지 전부 아직 안 봤다**

`npx granite dev`로 올린다. **저장소를 보는 1·8번은 반드시 전체 리로드로** —
fast refresh면 모듈 상태가 살아남아 저장이 깨져 있어도 통과한 것처럼 보인다.
예전에 그래서 몇 주를 놓쳤다.

각 줄 끝의 `[ ]`에 결과를 적는다. **고칠 게 나오면 그 줄 밑에 무엇을 어떻게 바꿨는지 남긴다.**

### 되는지만 보면 되는 여섯

- [ ] **1. 첫 실행** — 앱 열기 → 마이크 알림이 **안** 뜨는가(iOS는 마이크가 있다. 뜨면 그게 버그다).
      콤보 탭이 **빈 목록**(`눌러서 첫 콤보를 녹음해`)인가. **껐다 켜도 v1 콤보가 안 살아나는가**
- [ ] **3. 8초 상한** — 8초 넘게 말하기 → 저절로 완료되는가(`RECORD_MAX_MS`)
- [ ] **4. 이름 필수** — 이름 없이 저장 → `이름을 적어줘.` · 이름 적고 저장 → 맨 위 카드, 무대는 쉼으로
- [ ] **5. 카드 메뉴 넷** — 듣기 · **이름 고치기**(이름만 바뀌고 녹음은 그대로인가) ·
      **다시 녹음**(취소하면 원래 녹음이 남는가) · 삭제 → 되돌리기(녹음까지 돌아오는가)
- [ ] **6. 훈련** — 시작 → 내 목소리가 나오고 화면은 이름 + 막대.
      일시정지 → 재생이 **즉시** 끊기는가(기획서 6장). 건너뛰기. 앱 나갔다 오면 멈춰 있는가
- [ ] **9. 키보드** — 저장 FAB이 키보드 위로 따라 올라오는가. 이름 입력에 포커스가 오는가

### 답에 따라 코드가 바뀌는 셋 — **숫자를 들고 와라**

- [ ] **2. 앞뒤 자르기 문턱** — 녹음 → `듣기`.
      **첫 마디가 안 잘리고 뒤 침묵이 잘리는가?**
      첫 마디가 잘리면 문턱을 **내리고**, 앞 침묵이 길게 남으면 **올린다.**
      지금 5% — `VoiceEngine.tsx`의 `edges()` 안 `peak * 0.05`. 기획서 6장에 근거가 있다.
      앞뒤 여유는 따로다(`CLIP_LEAD` 0.15 / `CLIP_TAIL` 0.3, `utils/play.ts`)
- [ ] **7. 템포 음정** — 설정에서 0.8과 1.3을 걸고 들어본다.
      **음정 변화가 참을 만한가?** 아니면 범위를 좁히거나(예 0.9~1.2) 템포를 아예 뺀다.
      지금 `SettingsSheet.tsx`의 `min 0.8 max 1.3 step 0.05`. 기획서 9장이 보류로 잡아 둔 자리다
- [ ] **8. 저장소 한도** — **콤보 20개**를 녹음하고 껐다 켠다.
      다 남는가? 켜질 때 눈에 띄게 늦지 않는가(앱이 켜질 때 본체를 전부 푼다 — 기획서 6장)?
      8초 × 25KB = 200KB, 20개면 4MB. **토스 저장소 한도는 문서에 안 적혀 있다.**
      늦으면 "켤 때 전부 풀기"를 "부를 때 풀기"로 바꾸는 것이 후보인데, 그러면 첫 콤보가 늦는다
...
- **C. 콤보 추가** — v2가 곧 이것이다. 1·2번이 닫혔으니 **기기에서 4번을 통과하면 같이 닫힌다.**
