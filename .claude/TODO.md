...
**남은 건 저장소 하나다.** 이것만 통과하면 병합이다.

### 백그라운드에서도 라운드가 돌고 목소리가 나온다 (2026-09-06 기기 확인)

**미니앱을 백그라운드로 보내도 타이머가 계속 가고 TTS가 계속 나온다.**
쉐도우는 폰을 내려놓고 하는 운동이라 이게 안 되면 본 기능이 끊기는 것이었는데, 실물에서 된다.

- **백그라운드 대책을 따로 만들지 마라.** 백그라운드 태스크·알림·오디오 세션 손질 같은 제안은
  풀 문제가 없다. 지금 `setTimeout` 사슬(`useCallouts`·`useTraining`)이 그대로 돈다.
- 이건 저장소 항목을 대신하지 않는다. 백그라운드로 갔다 **돌아온** 것이지 앱이 죽은 게 아니다.
  메모리에 값이 그대로 있으니 `flushMaterial`이 실제로 썼는지는 아직 안 밝혀졌다.

어느 OS에서 봤는지는 안 적혀 있다. iOS와 안드로이드는 백그라운드 정책이 다르므로
**한쪽만 봤다면 나머지 한쪽은 아직 모르는 것이다.**
