...
src/
  ...
  commons/                    화면에 매이지 않은 것
    ...
    types.ts    도메인 모델
    test-support/tdsMock · routerMock · storageMock
  screens/ShadowCoach/        `/` 라우트. 유일한 라우트다.
    index.tsx   화면 조립. 두 뷰를 갈아 끼우고 떠 있는 층과 시트를 얹는다.
...
**2026-09-13에 기획이 v2로 바뀌었다 — TTS 코치에서 내 목소리 코치로.**
**2026-09-15에 지우기 커밋이 끝나 코드가 v2 뼈대로 섰다** — 파서·동작·호출어 탭·두드리기·슬롯·
동작 고르기 시트·TTS가 전부 나갔다. 탭은 훈련·콤보 둘이고, 콤보는 이름과 녹음만 갖는다.

**지금은 뼈대다.** 무대는 녹음을 켜고 끄고 이름을 받지만, `head`/`tail`을 실제로 재는 것과
카드 메뉴의 완전한 배선은 [TODO.md](TODO.md) 2번이다. 관문 넷은 초록이다 —
`tsc` 0 · `eslint` 0 · `jest` 89/89 (10묶음) · `ait build` 0/0 양쪽 RN. **기기에서는 아직 안 봤다.**

포팅과 구조 분리는 끝났다. **하위 화면은 전부 바텀시트로 모였다** — 지금 시트는 설정 하나뿐이고,
...
**v1 시절 실기기 점검은 2026-09-06에 전부 닫혔다.** 거기서 넷이 나왔고 넷 다 고치고 다시 확인했다 —
토스트가 두 번째부터 안 접히던 것, 슬라이더가 손가락을 못 따라가던 것,
앱을 나가면 훈련이 조용히 멈추던 것(이제 일시정지한다. 기획서 8장),
그리고 **저장소가 통째로 날아가던 것**(`AsyncStorage` → 토스 `Storage`).
**그 넷의 해법은 v2에도 그대로 살아 있다.**

**남은 일은 [TODO.md](TODO.md)에 우선순위대로 있다. 새 작업을 시작하기 전에 거기를 먼저 읽어라.**
다음은 2번(녹음 무대·이름·카드) → 3번(훈련 화면) → 4번(기기 확인 아홉 가지)다.
스플래쉬(A)와 메인 컬러(D)는 여전히 열려 있고, D는 **"너무 동적"이 무엇인지** 답을 기다린다.
병합은 미뤘다. 같은 브랜치에서 이어서 한다.
