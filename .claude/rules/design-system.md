...
- `Toast`의 `duration`은 **ms가 아니라 초**다(`useTimer`가 `duration * 1000`으로 건다).
- `Toast`의 시계는 **마운트 때 한 번만** 걸린다(`useTimer`의 의존성 배열이 비어 있다).
  `open`을 다시 켜도 안 되감긴다 — 계속 떠 있는 토스트는 **두 번째부터 스스로 안 닫힌다.**
  띄울 때마다 바뀌는 `key`로 갈아 끼워야 한다.

**뒤의 둘은 `.d.ts`에 안 나온다.** 타입만 읽고 두 번 틀렸고 기기에서야 잡혔다.
**그 컴포넌트가 스스로 시계를 든다면 `.js` 본문까지 읽어라** — 단위와 되감기는 거기에만 있다.
