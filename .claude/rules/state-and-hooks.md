...
`@apps-in-toss/native-modules`의 `Storage`다. **`AsyncStorage`가 아니다.**

`@granite-js/native`가 `@react-native-async-storage/async-storage`를 그대로 재수출하고 타입도 맞아서
오래 그걸 썼는데, **미니앱을 완전히 종료하면 거기 넣은 것은 남지 않는다.**
기기에서 콤보와 호출어가 통째로 초기화되는 것으로 드러났다.
토스 `Storage`는 "앱이 종료되었다가 다시 시작해도 유지"라고 문서가 직접 말하는 자리다.

**타입이 맞는다고 그 위에서 산다는 뜻이 아니다.** 이 앱은 토스 안의 미니앱이고,
RN 생태계의 표준 모듈이 호스트에 실제로 붙어 있는지는 별개다.
새 네이티브 기능을 붙일 때 `@apps-in-toss`에 같은 것이 있는지 **먼저** 보라.
...
**대신 이 침묵은 위의 사고를 몇 주 동안 안 보이게 했다.** 쓰기가 통째로 실패해도 화면은 멀쩡하다.
사용자에게 조용한 건 맞다 — 그러나 **우리가 그걸 확인하는 길은 실기기에서 껐다 켜는 것뿐이다.**
저장 경로를 건드렸으면 그 확인을 미루지 마라.
